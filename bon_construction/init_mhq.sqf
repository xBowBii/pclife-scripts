...
if(typeOf player in _build_classes || count _build_classes == 0) then {
	...
	if(isNil "tfor_current_commander_funds") then{tfor_current_commander_funds = tfor_commanderfunds};

	_coin setvariable ["BIS_COIN_funds","tfor_current_commander_funds"];
	_coin setvariable ["BIS_COIN_fundsDescription","C"];
	_coin setvariable ["BIS_COIN_categories",["Commandpost","Support","Defense"]];
	_coin setvariable ["BIS_COIN_items",tfor_commander_assets];
	_coin setvariable ["BIS_COIN_areasize",[50,50]];
