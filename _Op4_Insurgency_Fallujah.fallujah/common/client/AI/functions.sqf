...
fillHouseEast = { 
	private ["_x","_process","_arr","_inc","_pID","_pos","_bool","_unit","_name","_class","_ai","_nPos","_house","_cCount","_hID","_wCount","_i","_group"];
	...
	for [{ _i=_x},{ _i<((_nPos-1)+_x)},{ _i=_i+_inc}] do {  
		...
		if (count nearestObjects[_pos, ["Man"], 3] == 0) then {
			...
			_ai setPosATL _pos;
//			sleep 1;
//			nul= [_ai] execVM "op4\skilltest.sqf";
//			player sidechat "ai spawn";
			call compile format['
				%1 = _ai;
				_ai setVehicleInit ''
					...
					%1 = this;
					doStop this;
					nul= [this] execVM "op4\initai.sqf";
...
//					this setSkill [''''aimingAccuracy'''',%3];
				...
				'';
			', _name, IEDList, aiSkill, spawnPos];
			_process = true;
		};
		sleep 0.1;
	};
