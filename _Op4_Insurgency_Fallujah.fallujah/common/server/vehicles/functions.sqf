westVehicleChecker = {
	...
	_pos = startLocation modelToWorld [6,-3,0];
	_pos set [2,0];
	...
	_pos = startLocation modelToWorld [6,4,0];
	_pos set [2,0];
	if (mhqbox distance _pos > 1) then { 
		mhqbox setPosATL _pos;
		mhqbox setDir (getDir startLocation);
	};	
