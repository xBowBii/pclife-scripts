...
spawnAIVehicles = { 
	private ["_num","_track","_speed","_grp","_type","_obj","_mkr","_pos","_vcl","_ai"];
    
    for "_i" from 1 to eastVehicleNum do {  
		_num	= _i % 3; 
		if (_num == 0) then { _num = 3; }; 
		_grp	= ["","vclGrp",_i+1,"east"] call getGroup; 
...
createRoofGun = { 	
	...
	_ai setskill ["aimingAccuracy",(0.15 + random 0.25)];
    _ai setskill ["spotDistance",0.85];
    _ai setskill ["spotTime",0.85];
    _ai setskill ["courage",0.85]; 
    _ai setskill ["commanding",0.85]; 
    _ai setskill ["aimingShake",0.15];
    _ai setskill ["aimingSpeed",0.65];
