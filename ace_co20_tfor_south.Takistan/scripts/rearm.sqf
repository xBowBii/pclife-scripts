...
// get the vehicle seat of the caller. Resupply aborts if the seat changes.
...
if(isNil "BON_REARM_GETCALLERSEAT") exitWith{hint "cargo can't resupply this vehicle."};
...
[_unit,_pos,_caller] spawn {
	...
	if(damage _unit == 0) then {_unit vehicleChat "Repairing complete!"}
	else{_unit vehicleChat "Repairing cancelled."};
...
[_unit,_pos,_caller] spawn {
	...
	if(fuel _unit >= 0.99) then {_unit vehicleChat "Refueling complete!"}
	else{_unit vehicleChat "Refueling cancelled."};
...
[_unit,_pos,_caller] spawn {
	...
	if(_turretcount > 0) then {
...

		{
			...
			// removing the magazine currently loaded
...

		{
			...
			_unit vehicleChat format["Rearming magazine %1...",_displayname];
	...
	if(_unit distance _pos < 2.75 && getPos _unit select 2 < 0.5 && _caller == _unit call BON_REARM_GETCALLERSEAT) then {
		_unit vehicleChat "Rearming complete!"
	}
	else {_unit vehicleChat "Rearming cancelled."};
