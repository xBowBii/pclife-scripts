...
if (_classNameToDrop == "reammobox") then  	
{
  ...
  if (tfor_friendly_side == east) then {_classNameToDrop = "RUBasicAmmunitionBox"}; //RULaunchersBox //RUOrdnanceBox //RUBasicWeaponsBox //RUSpecialWeaponsBox //RUVehicleBox
  if (tfor_friendly_side == resistance) then {_classNameToDrop = "GuerillaCacheBox"}; //SpecialWeaponsBox
};
...
{    
 ...
 if (tfor_friendly_side == east) then {_classNameToDrop = "UAZ_MG_INS"}; //RULaunchersBox //RUOrdnanceBox //RUBasicWeaponsBox //RUSpecialWeaponsBox //RUVehicleBox
 if (tfor_friendly_side == resistance) then {_classNameToDrop = "UAZ_MG_INS"}; //SpecialWeaponsBox    
};

_paraName = "";
switch (_paraSize) do 
{    
  _side = tfor_friendly_side;
  if (!(_side in ["East","West"])) then {_side = "West";};
  	case 0: {_paraName = format["Parachute%1_EP1",_side],;}; //ParachuteMediumEast or ParachuteMediumWest not "FLY" (that spawns 150 m above ground) }; 
  	case 1: {_paraName = format["ParachuteMedium%1_EP1",_side];}; //ParachuteMediumEast or ParachuteMediumWest not "FLY" (that spawns 150 m above ground) }; 
...
[_crate, _para,_classNameToDrop] spawn {
  ...
  if ((_crate isKindOf "ReammoBox")) then 
  {
	...
	_crate addEventHandler ["Killed",{(_this select 0) spawn {sleep 300; deleteVehicle _this}}];
  	
  	crateDebug = _crate;
  	//_crate setPos [_groundHitPos select 0, _groundHitPos select 1, 0.0];  	
  	//_cratePos = position _crate;
    
  	tfor_unloadedbox = _crate;
