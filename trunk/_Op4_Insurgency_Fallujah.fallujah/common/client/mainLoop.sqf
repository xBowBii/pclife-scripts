...
if isWest then {
	[] spawn { 
		...
		while { true } do {
			...
			hint "3rd person disabled while enemy players are present.";
			};
			}; 
			};
			sleep 0.1;
...
//	call oneShotM136; 
	...
	if (getMarkerPos "USFLAG" distance (vehicleServicePoint modelToWorld [10,0,0]) > 10) then {
		"USFLAG" setMarkerPosLocal (vehicleServicePoint modelToWorld [10,0,0]);
	};
