onKeyPress = { 
	...
	if (_key in actionKeys "tacticalView") then {clearradio; _handled=true; };

	if (playersnumber east > 0) then {
	if (vehicle player == player) then {
	if (_key in actionKeys "personView") then {hint "3rd person disabled while enemy players are present."; _handled=true; 
	}; 
	};
	};
