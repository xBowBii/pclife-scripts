	...
	_WCTime = time;
	{
		if (nearestPlayers(getPosATL _x,WEP_DESPAWN_RANGE,true,"count") == 0 
		 && nearestEastMen(getPosATL _x,WEP_DESPAWN_RANGE,true,"count") == 0) then { 
			...
			if (typeOf _x == "Land_BagFenceRound") exitWith { _x setPosATL spawnPos; };
			...
			_cargoTypes = (getWeaponCargo _x) select 0;
			for "_i" from 0 to (count _cargoTypes - 1) do {
				_wep = _cargoTypes select _i;
				if (_wep in limitedGearList) then { [_x,_wep] call setWeaponCargo; };
			};
		};	
		sleep 0.005;				
	} forEach nearestObjects[CENTERPOS,["CraterLong","Suitcase","WeaponHolder","ReammoBox","AllVehicles","Land_BagFenceRound"],AORADIUS];
};
