...
switch (_locality) do {
	case "player": {
		if (_name == name player) then {
			if(format["%1", HLOsDebug] == "true") then { hint format["onPlayerConnected.sqf:\nA player is connecting.\n\nLocality = %1\nName: %2\nID: %3\n\nThis has been executed on the connecting player.",_locality,_name,_id]; };
			// ************************************************************************************************************************************************************ //
			// This will be executed on the connecting player. Add your code below this line.
			
			
			// ************************************************************************************************************************************************************ //
		} else {
			if(format["%1", HLOsDebug] == "true") then { hint format["onPlayerConnected.sqf:\nA player is connecting.\n\nLocality = %1\nName: %2\nID: %3\n\nThis has been executed on all players excluding the connecting player.",_locality,_name,_id]; };
			// ************************************************************************************************************************************************************ //
			// This will be executed on all players excluding the connecting player. Add your code below this line.
