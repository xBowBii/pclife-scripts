...
_disableHouseAI = [_pad] call fixObstacles;
...
if (_disableHouseAI) then {_target disableAI "MOVE"; _inBuilding = " in building";};

if (_tDistance < 1000) then {[-2, {player globalchat _this}, format ["New target: man, %1, closer than 1km%2",_tDir,_inBuilding]] call CBA_fnc_globalExecute;};
if (_tDistance > 1000) then {[-2, {player globalchat _this}, format ["New target: man, %1, farther than 1km%2",_tDir,_inBuilding]] call CBA_fnc_globalExecute;};
