...
HW_arti_number_shells_per_hour = 32;

// element: [displayname,config type-entry]
HW_arti_types = [["HE","ARTY_Sh_105_HE"],["SADARM","SADARM"],["DPICM","Sh_85_HE"]];

// max. nr. shells each cannon can fire in one fire mission
HW_arti_maxnrshells = 4;
...
_arti_cond_other = "{(typeOf vehicle _x == tfor_artitype)} count (units group player) > 0";
...
if(call compile _arti_cond_side && call compile _arti_cond_class) then {
