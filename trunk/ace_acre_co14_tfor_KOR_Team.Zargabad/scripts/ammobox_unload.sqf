...
if(if(isNil "_boxloaded") then {true} else {not _boxloaded}) then{
	...
	if(getPos _vec select 2 > 2 && getPos _vec select 2 < 160) exitWith {hint "either LAND or CLIMB >160 м для того, чтобы выгрузить."};

	if(getPos _vec select 2 < 2) then {
		if(abs (speed _vec) > 1) then {
			hintSilent "Слишком быстро, нельзя выгрузить.";
		} else {
