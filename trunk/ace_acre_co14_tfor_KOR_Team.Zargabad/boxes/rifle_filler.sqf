if (isServer) then {
  riflebox = _this select 0;
//  rifleBox = createVehicle ["RUVehicleBox", getMarkerPos _point, [], 0, "CAN_COLLIDE"];
  publicVariable "rifleBox";
};
...
while {alive rifleBox} do {

    {deleteVehicle _x} foreach nearestObjects [position rifleBox,["WeaponHolder"],100];

    clearMagazineCargo rifleBox;
    clearWeaponCargo rifleBox;

    {
      rifleBox addWeaponCargo [_x, 2];
    } foreach _weapons;

    {
        rifleBox addMagazineCargo [_x, 50];
    } foreach _magazines;
